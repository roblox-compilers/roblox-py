...
def version():
    print(
        ...
        + VERSION
        + "\033[0m"
        + " licensed under the GNU Affero General Public License by "
        + "\033[1m"
        + "@AsynchronousAI"
        + "\033[0m"
    )
...
def main():
    ...
    if type == 1:
        ...
        if (not Path(input_filename).is_file()) and not reqfile:
            error("The given filename ('{}') is not a file.".format(input_filename))
        ...
        if reqfile:
            ...
            if not notebook:
                ...
                if pyright and "-c" not in args:

                    def check():
                        os.environ["PYRIGHT_PYTHON_FORCE_VERSION"] = "latest"
                        success = (
                            subprocess.Popen(["pyright", input_filename]).wait() == 0
                        )
                ...
                lua_code = translator.translate(
                    content, includeSTD, False, export, False, useRequire, pyright
                )
            else:
                ...
                for i, cell in enumerate(cells):
                    ...
                    if cell["cell_type"] == "code":
                        ...
                        code += '\n""" MD:\n\t' + "\t".join(cell["source"]) + '\n"""\n'
                lua_code = translator.translate(
                    code, includeSTD, False, export, False, useRequire, False
                )
